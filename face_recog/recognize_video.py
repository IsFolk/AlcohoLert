...
if carstatus.getStatus() == 2:
    while True:
        if carstatus.getStatus() == 3:
            cv2.destroyAllWindows()
            subprocess.call("./reset_system.sh", shell = True)
            break
	...
	for i in range(0, detections.shape[2]):
		...
		if confidence > args["confidence"]:
			...
			if name not in name_list:
				name_list[name] = {"total":0, "count":0}
			else:
				name_list[name]["total"] = name_list[name]["total"] + proba * 100
				name_list[name]["count"] += 1
			...
			if end - start > 30:
				...
				if name != "test":
					...
					carstatus.uploadCarStatus(3)
					pbn.confirmed_notify()
					cv2.destroyAllWindows()
					subprocess.call("./reset_system.sh", shell = True)
					#os.system("python3 reset_system.py --cascade /usr/local/share/opencv4/haarcascades/haarcascade_frontalface_default.xml")
					#exec(open('reset_system.py').read())
					#exit()
